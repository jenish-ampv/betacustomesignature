	...
	private function GetUserProfilePic(){
		if(is_numeric($GLOBALS['USERID'])){
			...
			if($usrRes['user_image'] != 'default.png' ){
				...
				$GLOBALS['USERPROFILEIMG'] = '<div class="size-9 md:size-8 rounded-lg overflow-hidden text-white flex items-center justify-center"><img class="w-full h-full object-cover" src="'.$profile_img.'?'.time().'" alt=""></div>';
			}else{
				$profile_txt = substr($GLOBALS['USERNAME'],0,1);
				$GLOBALS['USERPROFILEIMG'] = '<div class="size-9 md:size-8 rounded-lg overflow-hidden bg-gradient text-white flex items-center justify-center"><span class="profiletxt">'.$profile_txt.'</span></div>';
