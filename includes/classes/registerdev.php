...
class CIT_REGISTERDEV
{
	...
	public function displayPage(){
		...
		if($_REQUEST['category_id'] == 'plan' && is_numeric($_REQUEST['plan_id']) && is_numeric($_REQUEST['plan_unit'])){
			...
			GetFrontRedirectUrl(GetUrl(array('module'=>'registerdev')));
		}
			 ...
			 if($_POST['stripeToken']!="" && $_POST['user_email'] !="" && $_POST['user_firstname'] !="" && $_POST['plan_id'] !="" && $_POST['plan_unit'] !=""){
				if($this->emailExist($_POST['user_email']) == false){
					...
					if($Subscription){	
							...
							$data =array('user_googleid'=>$user_googleid,'user_outhprovider'=>$user_outhprovider,'user_firstname'=>trim($_POST['user_firstname']),'user_lastname'=>trim($_POST['user_lastname']),'user_email'=>trim(strtolower($_POST['user_email'])),'user_password'=>$user_password,'user_organization'=>trim($_POST['user_organization']),'user_image'=>$user_image,'user_ip'=>$_SERVER['REMOTE_ADDR'],'user_planactive'=>1);
		...
		$GLOBALS['STRIPE_PUBLISHABLE_KEY'] = 'pk_test_51Lf5JtHp7DgNkCIvskUjNu75xBD5bJCrsAgaPf0ZT9al8jfTzcJXG8TwD1NZMI03qqINPFFIrrYHuJSN5n9ubfD100czetLSeB';
		$GLOBALS['CLA_HTML']->addMain($GLOBALS['WWW_TPL'].'/registerdev.html');	
		$GLOBALS['HEADER'] = $GLOBALS['CLA_HTML']->addSub($GLOBALS['WWW_TPL'].'/page.header.html');			
		$GLOBALS['FOOTER'] = $GLOBALS['CLA_HTML']->addSub($GLOBALS['WWW_TPL'].'/page.footer.html');
		$GLOBALS['SIDEBAR'] = $GLOBALS['CLA_HTML']->addSub($GLOBALS['WWW_TPL'].'/page.sidebar.html');
		$GLOBALS['CLA_HTML']->display();
	...
	public function getPlanDetail($selplan_id='',$selunit='',$getunit =1){
		...
		foreach($planRows as $planRow){
			...
			if($planRow['plan_id'] == $selplan_id){
					 ...
					 if($planRow['plan_id'] == 1){ // basic Q
					 	$GLOBALS['plan_priceid'] = 'price_1MpmUtHp7DgNkCIvZI242Ytp';
					 }else if($planRow['plan_id'] == 2){ // pro Q
						 $GLOBALS['plan_priceid'] = 'price_1MpmYjHp7DgNkCIvzvmtF11c';
					 }else if($planRow['plan_id'] == 3){ // basic Y
						 $GLOBALS['plan_priceid'] = 'price_1MpmbVHp7DgNkCIvNRnp9940';
					 }else if($planRow['plan_id'] == 4){ // pro Y
						 $GLOBALS['plan_priceid'] = 'price_1MpmeMHp7DgNkCIvcqUxHJQQ';
					 }
	...
	public function createSubscription($token,$plan_id,$plan_unit){
		...
		\Stripe\Stripe::setApiKey('sk_test_51Lf5JtHp7DgNkCIvp1WlMJ4NaRzuyCiqQFmvpZKvYToQuk0XpyFpBDEPMd10mQLTNsIHih4nHdhop3PknZ3FU4XF004Y7uuWlQ');
		...
		echo $GLOBALS['Error']= $api_error; exit;
		return false;
	}
	...
	public function addStripeSubscriptionData($subsData){
		...
		if($memRow['user_id']){
			$data = array('plan_id'=>$planId,'customer_id'=>$customer_id,'subscription_id' => $subscription_id,'price_id' =>$plan_id,'plan_interval' => $plan_interval,'plan_signaturelimit'=>$GLOBALS['plan_signaturelimit'],'period_start' => $start_time,'period_end' => $end_time,'apply_coupon'=>$coupon_id,'invoice_amount'=>$amount_paid,'invoice_link' => $invoice_link);
	...
	private function StripeWebhook(){ 
		\Stripe\Stripe::setApiKey('sk_test_51Lf5JtHp7DgNkCIvp1WlMJ4NaRzuyCiqQFmvpZKvYToQuk0XpyFpBDEPMd10mQLTNsIHih4nHdhop3PknZ3FU4XF004Y7uuWlQ');
		$endpoint_secret = 'we_1LfLcxHp7DgNkCIvA7NVKqhB';  // test secret
		...
		switch ($event->type) {
			case 'invoice.payment_succeeded': // subscription create success
				...
				if($memRow['user_id']){
					$data = array('plan_id'=>$planId,'customer_id'=>$customer_id,'subscription_id' => $subscription_id,'price_id' =>$plan_id,'plan_interval' => $plan_interval,'period_start' => $start_time,'period_end' => $end_time,'invoice_link' => $invoice_link,'invoice_amount'=>$amount_paid);
					$where = array('user_id'=>$userId);
					$add = $GLOBALS['DB']->update('registerusers_subscription',$data,$where);
					
				}else{
					$data = array('plan_id'=>$planId,'customer_id'=>$customer_id,'subscription_id' => $subscription_id,'price_id' =>$plan_id,'plan_interval' => $plan_interval,'period_start' => $start_time,'period_end' => $end_time,'apply_coupon'=>$coupon_id,'invoice_link' => $invoice_link,'invoice_amount'=>$amount_paid);
					$add = $GLOBALS['DB']->insert("registerusers_subscription",$data);
				}
