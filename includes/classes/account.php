			...
			if($this->emailExist($_POST['user_email']) == false){
				if($_POST['user_firstname'] !=""){
					...
					if($usrRes['user_image'] != 'default.png'){
						...
						$GLOBALS['USERPROFILEIMG'] = '<div class="size-9 md:size-8 rounded-lg overflow-hidden text-white flex items-center justify-center"><img src="'.$profile_img.'?'.time().'" alt=""></div>';
					}else{
